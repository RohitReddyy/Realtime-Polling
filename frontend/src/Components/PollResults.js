...
const PollResults = () => {
  ...
  const fetchPollHistory = async () => {
    try {
      const response = await fetch(`https://realtime-polling-frontend.vercel.app/api/polls/history/active/${userId}`);
      if (response.ok) {
  ...
  const fetchPollPercentages = async (pollId) => {
    try {
      const response = await fetch(`https://realtime-polling-frontend.vercel.app/api/pollResponses/${pollId}`);
      if (response.ok) {
        const data = await response.json();
  ...
  const endPolling = async (pollId) => {
    try {
      const response = await fetch(`https://realtime-polling-frontend.vercel.app/api/polls/end/${pollId}`, {
        method: 'PUT',
        headers: {
