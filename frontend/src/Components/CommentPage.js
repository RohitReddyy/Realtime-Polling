...
const CommentPage = () => {
  ...
  useEffect(() => {
    const newSocket = io('https://realtime-polling-api.vercel.app/');
    setSocket(newSocket);
...
window.location.href = '/studentdashboard';      }
  ...
  useEffect(() => {
    const fetchPoll = async () => {
      try {
        const response = await fetch(`https://realtime-polling-api.vercel.app/api/polls/${pollId}`);
        if (response.ok) {
  ...
  useEffect(() => {
    const fetchComments = async () => {
      try {
        const response = await fetch(`https://realtime-polling-api.vercel.app/api/comments/${pollId}`);
        if (response.ok) {
          const data = await response.json();
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    try {
      const response = await fetch('https://realtime-polling-api.vercel.app/api/comments', {
        method: 'POST',
        headers: {
      ...
      try {
        ...
        const usernameData = await Promise.all(
          userIds.map(async userId => {
            const response = await axios.get(`https://realtime-polling-api.vercel.app/api/studentauth/getusername/${userId}`);
